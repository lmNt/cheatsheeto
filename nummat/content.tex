...
\begin{align*}
\v{A}^k\v{x}^{(0)} = \v{A}^k\v{C}\v{v}^{(0)} = \v{C}\widehat{\v{A}}^k\v{v}^{(0)} =  \sum_{j=1}^n c_j \v{A}^k \v{v}_j=\sum_{j=1}^n c_j \lambda_j^k \v{v}_j
\end{align*}
Also folgt:
\begin{align*}
...
%Mit Hauptachsentransformation auf Diagonalgestalt bringen, sodass eine orthogonale Matrix $\v{U} \in \Rr{n}{n}$ existiert, wo $\v{U}^{-1}\v{AU}=\v{D}$ gilt, sodass die Eigenwerte $\lambda_n$ in absteigendem Betrag auf der Diagonalen von $\v{D}$ liegen. $\v{A}$ besitzt somit nur reelle EW und $n$ LU zueinander orthogonale Eigenvektoren (Spalten von $\v{U}$).\\
